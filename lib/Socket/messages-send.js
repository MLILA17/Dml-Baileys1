...
const makeMessagesSocket = (config) => {
    ...
    const DmlHandler = new Dml(Utils_1, waUploadToServer, relayMessage, config, sock);
    ...
    return {
        ...
        sendStatusMention: async (content, jids = []) => {
            return await DmlHandler.sendStatusWhatsApp(content, jids);
        },
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = DmlHandler.detectType(content);
            if (typeof content === 'object' && 'disappearingMessagesInChat' in content &&
                ...
                if (messageType) {
                    switch (messageType) {
                        case 'PAYMENT':
                            const paymentContent = await DmlHandler.handlePayment(content, quoted);
                            return await relayMessage(jid, paymentContent, {
                                messageId: Utils_1.generateMessageID(),
                                ...getParticipantAttr()
                            });
                        case 'PRODUCT':
                            const productContent = await DmlHandler.handleProduct(content, jid, quoted);
                        ...
                        case 'INTERACTIVE':
                            const interactiveContent = await DmlHandler.handleInteractive(content, jid, quoted);
                        ...
                        case 'ALBUM':
                            return await DmlHandler.handleAlbum(content, jid, quoted)
                        case 'EVENT':
                            return await DmlHandler.handleEvent(content, jid, quoted)
                        case 'POLL_RESULT':
                            return await DmlHandler.handlePollResult(content, jid, quoted)
                        case 'GROUP_STORY':
                            return await DmlHandler.handleGroupStory(content, jid, quoted)
                    }
                }
